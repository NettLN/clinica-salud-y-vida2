...
import { Clipboard, List, Play, CheckCircle, Clock, FileText, UploadCloud, X, AlertTriangle, User } from 'lucide-react';
...
// 2FA ya fue validado en Login.jsx
  ...
  const [tareaEnProceso, setTareaEnProceso] = useState(null); // La tarea que el enfermero está haciendo AHORA
  ...
  // Actualizador de timers (cada minuto)
  ...
  const fetchTareas = async () => {
    try {
      ...
      setTareas(data);
      // Restaurar tarea en proceso si existe
  ...
  // Separar Tareas Disponibles vs Otras
  const tareasDisponibles = tareas.filter(t => t.estado === 'Pendiente_General' || t.estado === 'Pendiente_Asignada');

  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
      <nav className="bg-teal-600 shadow-lg px-6 py-4 flex justify-between items-center text-white">
        <div className="flex items-center space-x-3">
          <div className="bg-white/20 p-2 rounded-lg"><Clipboard size={24} /></div>
          <div>
            <h1 className="text-xl font-bold">Estación de Enfermería</h1>
            <p className="text-xs font-semibold uppercase tracking-wider">Enf. {user?.nombre} {user?.apellido}</p>
          </div>
        </div>
        <button onClick={logout} className="bg-teal-700 hover:bg-teal-800 p-2 rounded-full transition">
          <User size={20} />
        </button>
      </nav>

      <main className="flex-1 p-6 max-w-7xl mx-auto w-full flex flex-col lg:flex-row gap-6">
        ...
        {/* ÁREA DE TRABAJO ACTIVO (Tarea en Proceso) */}
        <section className="w-full lg:w-1/3 flex flex-col gap-6">
          <div className="bg-white p-6 rounded-3xl shadow-lg border border-teal-100 flex-1 relative overflow-hidden">
            {/* Decoración de fondo */}
            ...
            {!tareaEnProceso ? (
              ...
              <div className="flex flex-col h-full relative z-10">
                ...
                {/* Acceso a Historial Clínico (Si está permitido) */}
                ...
                {/* Envío de Evidencia y Finalización */}
                <div className="mt-auto">
                  ...
                  <textarea 
                    ...
                    placeholder="Escribe notas, observaciones, o adjunta links..."
